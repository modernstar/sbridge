...
static FUNC_DUMP(dump_llc)
{
	/*
	 * Low layer compatibility IE (may be repeated)
	 * len = how many octets in the IE including the IE identifier octet and the IE length octet
	 * however data begins at octet 2 (data[0]) 
	 * we get 8 octets from the telco 7c 06 88 90 21 48 40 bb
	 * 01111100 - IE identifier for Low layer compatibility (see Q931 4.5.19)
	 * 00000110 - 6 octets of length (following octets)
	 * 10001000 - octet 3 - index 0 - National Standard - Unrestricted digital information
	 * 10010000 - octet 4 - index 1 - Circuit mode - 64kbit/s
	 *
	 * 00100001 - octet 5 - index 2 - Layer 1 - V.110
	 * 01001000 - octet 5.a - index 3 - Asynchronous data - In-band negotiation not possible - 9.6Kbit/s recommendation X.1
	 * 01000000 - octet 5.b - index 4 - Rate adaption Hdr included - Only UI frames allowed - 
	 * 			            Bit transparent mode - LLI = 256 only - default assingee - 
	 * 			            Negotiation done USER_INFORMATION on temporary signalling connection
	 * 10111011 - octet 5.c - index 5 - 1 stop bit - 8 data bits excluding parity bit - No partity bit
	 * */
	int pos = 2;
	pri_message(pri, "%c Low Level Compatibility (len=%2d) [ Ext: %d  Q931 Std: %d  Info transfer capability %s (%d) ]\n", 
		prefix, len, (ie->data[0] & 0x80) >> 7, (ie->data[0] & 0x60) >> 5, cap2str(ie->data[0] & 0x1f), ie->data[0] & 0x1f);
	...
	/* Look for octet 5; this is identified by bits 5,6 == 01 */
#define LLC_LAYER_2_ID 0x20
     	if (pos < len &&
		(ie->data[pos] & 0x60) == LLC_LAYER_2_ID) {
...
static struct ie ies[] = {
	...
	{ 0, Q931_LOW_LAYER_COMPAT, "Low-layer Compatibility", dump_llc, receive_llc },
